...
const CommunityMembers = ({ userId }: { userId: string }) => {
  ...
  const { data: communityExists, isLoading: communityExistsLoading } =
    useQuery<{ exists: boolean; creatorId: string }>({
      queryKey: ["community-exists", communityId],
      queryFn: async () => await verifyCommunityExists(communityId),
    });

  const communityCreator = communityExists?.creatorId as string;
  ...
  return (
    <Card className="rounded-none hidden lg:inline lg:w-[30%] xl:w-[27%] top-0 bottom-0 fixed right-0 overflow-y-auto transition-all duration-500 border-t-0 border-r-0 border-b-0">
      {communityExistsLoading ? (
        <Loader2 className="w-5 h-5 animate-spin mt-[50vh] mx-auto flex items-center justify-center" />
      ) : (
